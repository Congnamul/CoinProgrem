...
public class Coin : MonoBehaviour
{
    ...
    public void UpdateCoin()
    {
        ...
        for(int i = 0; i < coinDatas.Length; i++)
        {
            ...
            //현재 주식이 이슈상태라면 - 폭등/폭락
            if (coinDatas[i].issueData[0])
            {
                //상승 하락 bool값에 따라 결과가 나뉨
                coinDatas[i].lastUpdate = (coinDatas[i].issueData[1]) ?
                Random.Range(coinDatas[i].issueUp[0], coinDatas[i].issueUp[1]) :
                -Random.Range(coinDatas[i].issueUp[0], coinDatas[i].issueUp[1]);
                
                //만약 폭락했는데 0원 이하라면 100원으로 변경
                if( (coinDatas[i].nowCost - coinDatas[i].lastUpdate) > 0 )
                {
                    coinDatas[i].lastUpdate = 100 - coinDatas[i].nowCost;
                    coinDatas[i].nowCost = 100;
                }
                else
                {
                ...
                coinDatas[i].issueData[0] = false;
                coinDatas[i].issueData[1] = false;
                continue;
            }
